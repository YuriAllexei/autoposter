...
# ---- user-rule guards belong to Config itself (2026-09-22 refactor) ----


def test_direct_construction_is_capped_too():
    # no wait may exceed MAX_ALLOWED_DELAY even for a Config() built by hand
    # (not via load_config) — the guard moved to Config.__post_init__
...
def test_direct_construction_rejects_bad_post_as():
    import pytest
    with pytest.raises(ValueError, match="AP_POST_AS"):
        Config(post_as="who-knows")
